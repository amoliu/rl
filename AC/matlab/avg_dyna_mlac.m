...
for power=0:power_of_two
    ...
    parfor i=1:trials
        [~, ~, temp_cr, temp_rmse] = dyna_mlac_pendulum('mode', 'episode', 'episodes', episodes, 'steps', step);
        
        filename = strcat('dyna-mlac-', num2str(step), '-', num2str(trials), '-iterations-', num2str(episodes), '-episodes-', num2str(i));
        parsave(strcat(path, filename), temp_cr);
    
        filename = strcat('dyna-mlac-rmse', num2str(step), '-', num2str(trials), '-iterations-', num2str(episodes), '-episodes-', num2str(i));
