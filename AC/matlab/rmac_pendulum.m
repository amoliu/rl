function [critic, actor, cr, rmse] = rmac_pendulum()
    % Initialize simulation
    spec = env_mops_sim('init');

    % LLR parameters
    llr_params = 2*spec.observation_dims + spec.action_dims + 2;

    % Initialize parameters
    critic.memory  = 2000;       % LLR memory size
    critic.llr     = zeros([critic.memory llr_params]);
    critic.k       = 20;         % Nearest neighbours used in LLR
    critic.weight  = [1 0.1];    % Wc used in LLR
    critic.alpha   = 0.1;
    
    model.memory   = 100;
    model.llr     = zeros([model.memory llr_params]);
    model.k        = 9;
    model.weight   = [1 0.1 1];
    
    refmodel.memory = 2000;
    refmodel.llr    = zeros([refmodel.memory llr_params]);
    refmodel.alpha  = 0.005;
    refmodel.k      = 20;
    refmodel.weight = [1 0.1];
